<template>
    <div class="receivableBill">
        <header>应收账单
            <div>
                <router-link :to="{name:'bill', params:{type:'new',id:'1'}}">添加应收账单</router-link>
                <a href="#">选择应收账单文件</a>
            </div>
        </header>
...
//                console.log(this.code);
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                    ...
                    this.$http.post('/api/financing/applyWithdraw', this.qs.stringify(delObj)).then((res) => {
                        ...
                        if (res.data.code === 200) {
                            ...
                            //删除成功的时候请求的数据----------------------------------------------------
                            _this.$http.post('/api/receivables/queryReceivablesList', _this.qs.stringify(_this.listData[_this.code])).then((res) => {
            ...
            handleCurrentChange(val) {
                ...
                obj.page = val;
                this.$http.post('/api/receivables/queryReceivablesList', this.qs.stringify(obj)).then((res) => {
        ...
        created: function () {
//            //全部列表请求
            this.$http.post('/api/receivables/queryReceivablesList', this.qs.stringify(this.listData[this.code])).then((res) => {
                console.log(res);
                this.from_list_all = res.data.data.allList;
